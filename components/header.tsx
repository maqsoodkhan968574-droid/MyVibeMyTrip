...
export async function Header() {
  ...
  const accountLabel = session?.user ? "Account" : "Sign in / Sign up";
    ...
    <header className="sticky top-0 z-40 border-b border-slate-100 bg-white/95 backdrop-blur">
      <div className="container-shell flex h-16 items-center justify-between gap-3 sm:h-20 sm:gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link href={accountHref} className="hidden items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-navy transition hover:border-green-300 hover:text-green-700 sm:inline-flex">
            <UserRound size={18} />
            {accountLabel}
          </Link>
          ...
          <MobileNavigation accountHref={accountHref} accountLabel={accountLabel} />
        </div>
      </div>
    </header>
  );
